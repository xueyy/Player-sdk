...
(function (global, factory) {
  ...
  var ckplayerJS = Domain + 'https://cdn.jsdelivr.net/gh/xueyy/Player-sdk@master/ckplayer.js';
  var ckplayerSWF = Domain + '/static/ckplayer.swf';
  var m3u8SWF = Domain + '/static/m3u8.swf';
  var flv_js = Domain + '/static/flv.min.js';
  var hlsJS = Domain + 'https://cdn.jsdelivr.net/gh/xueyy/Player-sdk@master/hls.min.js';
  var mpegJS = Domain + '/static/jsmpeg.min.js';
                  ...
                  function recAPISuccess(data) {
                    if (data.code == 200) {
                      ...
                      if (data.data && data.data.files && data.data.files.length > 0) {
                        ...
                        _this.loadingSet(0, {
                          text: _this.opt.domain !== 'open' ? 'Get device live view address' : '获取设备播放地址'
                        })
                        resolve(JSON.stringify({
                          code: -1,
                          msg: _this.opt.domain !== 'open' ? 'Not found video clips' : "未找到录像片段"
                        }))
                        // reject('未找到录像片段');
                      }
                    } else {
                      ...
                      _this.loadingSet(0, {
                        text: _this.opt.domain !== 'open' ? 'Get device live view address complete' : '获取设备播放地址'
                      });
                      resolve(JSON.stringify({
                        code: -1,
                        msg: _this.opt.domain !== 'open' ? 'Not found video clips' : "未找到录像片段"
      ...
      var getRealUrlPromiseObj = Promise.all(promiseTaskList)
        .then(function (result) {
          ...
          result.forEach(function (item, index) {
            _this.loadingSet(index, {
              text: _this.opt.domain !== 'open' ? 'Get live view Address Success' : '获取播放地址成功'
            })
          })
        })
        .catch(function (err) {
        ...
        getRealUrl.then(function () {
          ...
          _this.opt.sources.forEach(function (item, index) {
            if (getQueryString('dev', item) || item.indexOf('ws') !== -1) {
              ...
              _this.loadingSet(index, {
                text: _this.opt.domain !== 'open' ? 'Ready to play...' : '准备播放...',
                color: '#fff'
              })
              ...
              _this.jSPlugin.JS_Play(wsUrl, wsParams, index).then(function () {
                ...
                _this.loadingSet(index, {
                  text: _this.opt.domain !== 'open' ? 'Played successfully...' : '播放成功...'
                });
                //单次播放日志上报
                ezuikitDclog({
                  systemName: PERFORMANCE_EZUIKIT,
  ...
  EZUIPlayer.prototype.stop = function (i, unDestory) {
    ...
    if (!!window['CKobject']) {
      ...
      var _this = this;
      var playParams = _this.playParams;
      if (typeof i === "undefined") {
        ...
        return this.jSPlugin.JS_Stop(0).then(function () {
          ...
          if (playParams.handleStop) {
            playParams.handleStop();
          }
        }, function () {
